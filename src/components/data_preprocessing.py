...
class DataPreprocessing:
    ...
    def audio_transformations(self) -> MelSpectrogram:
        ...
        transforamtion_object_path = self.data_preprocessing_config.transformations_object_path
        save_object(file_path=transforamtion_object_path, obj=mel_spectrogram)
    ...
    def initiate_data_preprocessing(self) -> DataPreprocessingArtifacts:
        ...
        try:
            ...
            data_preprocessing_artifacts = DataPreprocessingArtifacts(
                train_metadata_path=self.data_preprocessing_config.train_file_path,
            ...
            s3_sync.sync_folder_to_s3(folder=transformations_dir_path, aws_bucket_url=S3_ARTIFACTS_URI)
            logging.info("Finishing syncing files to S3 transformation artifacts folder")
